function INTAN2single_RHS2000_PARA(varargin)
...
if (num_amplifier_channels > 0)
    ...
    if (data_present)
        ...
        paths_FW = strrep(paths.FW, '\', '/');
        paths_FW_N = strrep(paths.FW_N, '\', '/');
        paths_CARW_N = strrep(paths.CARW_N, '\', '/');
        paths_DW = strrep(paths.DW, '\', '/');
        fname = cell(num_amplifier_channels,1);
        
        filt_infoname = fullfile(paths_FW,[Animal '_' Rec '_Filtspecs.mat']);
        save(filt_infoname,'filtspecs','gitInfo');

        parfor iCh = 1:num_amplifier_channels
            ...
            fname{iCh} = sprintf(paths_RW_N, pnum, chnum); %#ok<*PFOUS>
            ...
            if (dc_amp_data_saved ~= 0)
                if ~exist(fullfile(paths_DW,'DC_AMP'),'dir')
                    mkdir(fullfile(paths_DW,'DC_AMP'))
                end
                dc_amp_fname = strrep(fullfile(paths_DW,'DC_AMP',[Animal '_' Rec '_DCAMP_P%s_Ch_%s.mat']),'\','/');
                fname{iCh} = sprintf(dc_amp_fname, pnum, chnum); 
                data = single(dc_amplifier_data(iCh,:));
                parsave(fname{iCh},'data',data,'fs',fs,'gitInfo',gitInfo);
                data = [];
            end

            if ~exist(fullfile(paths_DW,'STIM_DATA'),'dir')
                mkdir(fullfile(paths_DW,'STIM_DATA'))
            end
            
            stim_data_fname = strrep(fullfile(paths_DW,'STIM_DATA',[Animal '_' Rec '_STIM_P%s_Ch_%s.mat']),'\','/');
            fname{iCh} = sprintf(stim_data_fname, pnum, chnum); 
            data = single(stim_data(iCh,:));
            parsave(fname{iCh},'data',data,'fs',fs,'gitInfo',gitInfo);
            data = [];
            
            as_data_fname = strrep(fullfile(paths_DW,'STIM_DATA',[Animal '_' Rec '_ASD_P%s_Ch_%s.mat']),'\','/');
            fname{iCh} = sprintf(as_data_fname, pnum, chnum); 
            data = single(amp_settle_data(iCh,:));
            parsave(fname{iCh},'data',data,'fs',fs,'gitInfo',gitInfo);
            data = [];
            
            cr_data_fname = strrep(fullfile(paths_DW,'STIM_DATA',[Animal '_' Rec '_CRD_P%s_Ch_%s.mat']),'\','/');
            fname{iCh} = sprintf(cr_data_fname, pnum, chnum); 
            data = single(charge_recovery_data(iCh,:));
            parsave(fname{iCh},'data',data,'fs',fs,'gitInfo',gitInfo);
            data = [];
            
            cl_data_fname = strrep(fullfile(paths_DW,'STIM_DATA',[Animal '_' Rec '_CLD_P%s_Ch_%s.mat']),'\','/');
            fname{iCh} = sprintf(cl_data_fname, pnum, chnum); 
        ...
        for iPb = 1:numel(probes)
            pnum  = num2str(probes(iPb));
            ch_nums = {amplifier_channels([amplifier_channels.port_number] == probes(iPb)).custom_channel_name};
            probe_ref(iPb,:) = mean(hold_filt([amplifier_channels.port_number] == probes(iPb),:),1);
            pref = probe_ref(iPb,:);
            CAR_data = hold_filt([amplifier_channels.port_number] == probes(iPb),:);
            fname = cell(size(CAR_data,1),1);
            parfor iCh = 1:size(CAR_data,1)
                chnum = ch_nums{iCh}(regexp(ch_nums{iCh}, '\d'));

                fname{iCh} = sprintf(paths_CARW_N, pnum, chnum);
                data = single(CAR_data(iCh,:) - pref);
                parsave(fname{iCh},'data',data,'fs',fs,'gitInfo',gitInfo);
                data = [];
            end
            clear pref CAR_data
        ...
        save(car_infoname,'probe_ref','gitInfo');
    end
end
...
% the length of the string (in bytes)_  If this number equals 0xFFFFFFFF,
% the string is null.
